...
import createTemplate, { Template } from '../../models/types/Template.ts';
...
export default function RubricBuilder(): ReactElement {
  ...
  const [lastSentRubric, setLastSentRubric] = useState<Template>(createTemplate()); // last rubric sent to server (for displaying in dialog)
  ...
  const handleRubricTitleChange = (event: ChangeEvent<HTMLInputElement>) => {
    const newRubric = { ...template };
    newRubric.title = event.target.value;
    setTemplate(newRubric);
  };
  ...
  const handleSubmitRubric = async (event: MouseEvent) => {
    ...
    const { exists, id } = await BackendAPI.checkTitleExists(template.title);
    if (exists) {
      // tell the user what happened
      setModalMessage(
        `A rubric with the title "${template.title}" already exists. How would you like to proceed?`
      );
      ...
      setModalChoices([
        {
          ...
          label: 'Overwrite',
          action: async () => {
            await BackendAPI.update(id, template).then(() => {
              setLastSentRubric(template);
              closeModal();
              openDialog();
            });
          },
        },
        {
          ...
          label: 'Make a Copy',
          action: async () => {
            const newRubric: Template = { ...template };
      ...
      await BackendAPI.create(template).then(() => {
        setLastSentRubric(template);
        openDialog();
  ...
  const handleImportFile = (data: any[]) => {
    ...
    setTemplate((prevTemplate) => ({
      ...prevTemplate,
      rubricCriteria: [...prevTemplate.rubricCriteria, ...newCriteria],
    }));
  };

  // function to iterate through each criterion and sum total max points for entire rubric
  const calculateTotalPoints = () => {
    const total: number = template.rubricCriteria.reduce(
  ...
  const handleAddCriteria = (event: MouseEvent) => {
    event.preventDefault();
    const newCriteria = [...template.rubricCriteria, createRubricCriterion()];
    // @ts-ignore
  ...
  const handleRemoveCriterion = (index: number) => {
    const newCriteria = [...template.rubricCriteria];
  ...
  const handleUpdateCriterion = (index: number, criterion: RubricCriterion) => {
    const newCriteria = [...template.rubricCriteria]; // copy criteria to new array
  ...
  const handleDragEnd = (event: { active: any; over: any }) => {
    if (event.over) {
      const oldIndex = template.rubricCriteria.findIndex(
        (criterion) => criterion.key === event.active.id
      );
      const newIndex = template.rubricCriteria.findIndex(
        (criterion) => criterion.key === event.over.id
      );

      const updatedCriteria = [...template.rubricCriteria];
      const [movedCriterion] = updatedCriteria.splice(oldIndex, 1);
      updatedCriteria.splice(newIndex, 0, movedCriterion);

      setTemplate({ ...template, rubricCriteria: updatedCriteria });
    }
  };
  ...
  const renderCriteria = () => {
    return (
      <SortableContext
        items={template.rubricCriteria.map((criterion) => criterion.key)}
        strategy={verticalListSortingStrategy}
      >
        {template.rubricCriteria.map((criterion, index) => (
          <CriteriaInput
            key={criterion.key}
            index={index}
