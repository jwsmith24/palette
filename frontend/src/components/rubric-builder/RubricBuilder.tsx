...
import {
    ...
    postRubric, updateRubricWithID, deleteRubricWithID
...
export default function RubricBuilder(): ReactElement {
    ...
    const handleSubmitRubric = async (event: MouseEvent) => {
        ...
        if (exists) {
            const overwrite = window.confirm(
                "A rubric with this title already exists. Do you want to overwrite it?",
            );
            // todo: expand this functionality to include the ability to make a new copy
            if (overwrite) {
                await updateRubricWithID(id, rubric);
                openDialog();
            }
        } else {
            await postRubric(rubric);
            openDialog();
        }
    };
    ...
    // Fires when drag event is over to resort criteria
    ...
    return (
        <DndContext onDragEnd={handleDragEnd}>
            <div
                className="min-h-screen justify-between flex flex-col w-screen bg-gradient-to-b from-gray-900 to-gray-700 text-white font-sans">
                ...
                <form
                    ...
                    {/* Criteria Section */}
                    <div
                        className="mt-6 grid gap-3 h-[35vh] max-h-[50vh] overflow-y-auto overflow-hidden scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-800">
                        {renderCriteria()}
                    </div>
                ...
                >
          <pre
              className="text-black bg-gray-100 p-4 rounded-lg max-h-96 overflow-auto">
